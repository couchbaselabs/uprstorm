...
public class Rankings implements Serializable {
  ...
  private void addOrReplace(Rankable r) {
    ...
    if (rank != null) {
      Rankable curr = rankedItems.get(rank);
      if ((r.getId() - curr.getId()) != 0){
  ...
  private void purgeStale(){
    List<Rankable> objsToRemove = Lists.newArrayList();

    for (Rankable r: rankedItems){
        Date now = new Date();
        long diff = (now.getTime() - r.getDate().getTime())/1000;
        if (diff > 10){
            objsToRemove.add(r);    
        }
    }

    for (Rankable r: objsToRemove){
