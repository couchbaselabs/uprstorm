...
import storm.starter.tools.ResultObj;
...
public class RollingCountBolt extends BaseRichBolt {
  ...
  private void emitCurrentWindowCounts() {
    Map<Object, ResultObj> counts = counter.getCountsThenAdvanceWindow();
  ...
  private void emit(Map<Object, ResultObj> counts, int actualWindowLengthInSeconds) {
    for (Entry<Object, ResultObj> entry : counts.entrySet()) {
      Object obj = entry.getKey();
      ResultObj result = entry.getValue();
      Long count = result.count;
      Long id = result.id;
      collector.emit(new Values(obj, count, id,  actualWindowLengthInSeconds));
    }
  }
  ...
  private void countObjAndAck(Tuple tuple) {
    ...
    counter.incrementCount(obj, id);
    collector.ack(tuple);
  }

  @Override
  public void declareOutputFields(OutputFieldsDeclarer declarer) {
    declarer.declare(new Fields("obj", "count", "id", "actualWindowLengthInSeconds"));
  }
